...
class TestInputProcessing < MiniTest::Test
  ...
  def test_input_processing_with_a_file_that_does_not_exist
    assert_raises(InputFileDoesNotExist) {Redbubble::XmlParser.new("#{@sample_data_folder_path}/kjhfjkdshfdjkfh.txt")}
  end

  def test_input_processing_with_a_xml_file_which_is_empty
    assert_raises(FileEmpty) {Redbubble::XmlParser.new("#{@sample_data_folder_path}/emptyfile.xml")}
  end

  def test_input_processing_with_a_non_xml_file
    assert_raises(FileNotXML) {Redbubble::XmlParser.new("#{@sample_data_folder_path}/some.html")}
  end

  def test_input_processing_with_yet_another_non_xml_file
    assert_raises(FileNotXML) {Redbubble::XmlParser.new("#{@sample_data_folder_path}/some.txt")}
  end

  def test_input_processing_with_a_xml_file_which_is_badly_formed
    assert_raises(Nokogiri::XML::SyntaxError) {Redbubble::XmlParser.new("#{@sample_data_folder_path}/badly_formed.xml")}
  end
